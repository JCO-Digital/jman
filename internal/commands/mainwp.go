...
var mainWPCmd = &cobra.Command{
	...
	RunE: func(cmd *cobra.Command, args []string) error {
		...
		for _, site := range sites {
			...
			if active {
				fmt.Printf("MainWP is already active for %s\n", site.Name)
				continue
			}

			fmt.Printf("Installing MainWP for %s\n", site.Name)
			var password string

			fmt.Println("Installing MainWP user...")
			pwd, err := wpcli.AddUser(site.SSH, site.Path, "mainwp", "mainwp@jco.fi", "administrator")
			if err != nil || pwd == "" {
				fmt.Printf("MainWP user already exists for %s, resetting password.\n", site.Name)
			...
			if err != nil || !success {
				fmt.Printf("MainWP Child Plugin failed to install for %s: %v\n", site.Name, err)
				continue
			}
			...
			if err := mainwp.AddSite(siteURL, "mainwp", password); err != nil {
				fmt.Printf("Error adding site to MainWP for %s: %v\n", site.Name, err)
				continue
			}

			fmt.Printf("Successfully installed and configured MainWP for %s\n", site.Name)
		}
