...
**Historical backlog on first deployment**: on the very first collection cycle for a site, there's often a backlog to work through — existing `access.log-*.gz` files (SpinupWP keeps months of rotated logs), and/or simply however many hours have already elapsed *today* in the not-yet-rotated live `access.log` (rotation only happens once a day, so this alone can be several hours' worth the first time the agent runs). The agent doesn't process all of it in one report: each cycle finalizes only up to a bounded number of hours, shared across every site on the server, keeping the request comfortably under jman-api's 1MB body limit regardless of backlog size. Any remainder is picked up on later cycles.

**Recent data is prioritized**: within that per-cycle budget, today's live log is always processed before any rotated backlog, and rotated days are walked most-recent-first. This means current/recent traffic should show up within the first few cycles even on a server with a huge historical backlog — you don't need to wait for months of old logs to drain before seeing today's numbers. Older history still backfills in the background at whatever rate the budget allows.
