...
Alongside the `jman` CLI, this repository includes `jman-api`, a lightweight, read-only REST API that serves the data cached by the CLI. This is useful for building dashboards or exposing your SpinupWP/Plugin data to other local services without needing to re-fetch from external APIs.
